...
class Bitrix:
	...
	def get_hours(self):
		for row in range(3, self.worksheet.max_row - 1):
			self.data[self.worksheet[row][1].value] = [self.worksheet[row][2].value, [
				dict([(int(self.worksheet[2][i].value[:4]), self.worksheet[row][i].value)]) for i in
				range(3, self.worksheet.max_row - 3)]]
		return self.data
