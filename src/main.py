...
def main():
	while True:
		...
		if data_from_user:
			...
			consultant = back.Consultant(data_from_user[2])
			excel = back.Excel(data_from_user[1], data_from_user[3])
			bitrix = back.Bitrix(data_from_user[4], data_from_user[1], data_from_user[2])
